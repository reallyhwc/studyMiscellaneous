 ...
 *     d: 存在等值目标元素但是为最后一位                         目标返回值   lastInde
...
public class TestMain {
    ...
    public static void main(String[] args) {
        FindLastTargetIndex find = new FindLastTargetIndex();
//        2：查找最后一个值等于给定值的元素 的下标
//        a: 所有元素都比这个目标元素大                            目标返回值    -1
...
//        e: 存在等值目标元素但是整个数组中前一大半都为等值目标元素  目标返回值 最后一个目标元素下标
        System.out.println(find.findIndex(new int[]{8,8,8,8,8,8,8,11,18},8));

//        f: 存在等值目标元素但是整个数组中后一大半都为等值目标元素  目标返回值   最后一个目标元素下标
        System.out.println(find.findIndex(new int[]{1,3,4,5,8,8,8,8,8,8},8));
...
//        i: 原始测试
        System.out.println(find.findIndex(new int[]{1,3,4,5,6,8,8,8,11,18},8));
    }
